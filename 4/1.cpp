...
int get_guard_with_highest_value(map<int, int> sl) {
    pair<int, int> max_key = {0, 0};
    for (auto el : sl) {
        if (el.second > max_key.second) max_key.first = el.first, max_key.second = el.second;
    }
...
void solve() {
    ...
    for (LogEntry log : logs) {
        if (log.event1 == "Guard") {
            ...
            if (sleep.find(current_guard) == sleep.end()) {
                ...
                sleep_sum.insert({current_guard, 0});
            }
            sleep_sum[current_guard] += (log.minute - asleep_time);
    ...
    int guard = get_guard_with_highest_value(sleep_sum);
    int idx = distance(sleep[guard].begin(), max_element(sleep[guard].begin(), sleep[guard].end()));
    int result = idx * guard;
